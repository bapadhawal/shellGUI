...
sudo sed -i.bak '/http[s]::proxy/Id' /etc/yum.conf
sudo tee -a /etc/yum.conf <<EOF
proxy=http://$USER_ID:$PASSWORD@$HTTP_PROXY_HOST:$HTTP_PROXY_PORT;
proxy=http://$USER_ID:$PASSWORD@$HTTPS_PROXY_HOST:$HTTPS_PROXY_PORT;
EOF
